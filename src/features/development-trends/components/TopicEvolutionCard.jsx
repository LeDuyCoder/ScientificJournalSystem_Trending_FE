...
export default function TopicEvolutionCard({ data, meta }) {
  const evolutionData = data || [];
  const evolutionMeta = meta || [];
        ...
        <div className="topic-evolution-wrapper" aria-label="Topic Evolution Stacked Area Chart">
          <ResponsiveContainer width="100%" height="60%">
            <AreaChart data={evolutionData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <Tooltip content={<CustomTooltip meta={evolutionMeta} />} cursor={{ stroke: 'var(--color-neutral-300)', strokeWidth: 1, strokeDasharray: '4 4' }} />
              {evolutionMeta.map(m => (
                <Area 
                  key={m.key}
                  type="monotone" 
